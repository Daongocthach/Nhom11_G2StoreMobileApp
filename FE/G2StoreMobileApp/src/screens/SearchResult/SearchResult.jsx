    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.body}>
        <View className='items-center mt-2 h-1/8 flex-row justify-between pl-5 pr-5 gap-2'>
          ...
          <View className='flex-row items-centerm pl-2 h-5/6 w-4/6 rounded-3xl' style={{ borderColor: '#828282', borderWidth: 1 }}
            onPress={() => navigation.navigate('SearchResult')}>
            <TextInput style={styles.input} className="text-black" placeholder='Search...' autoFocus value={input} onChangeText={(value) => { setInput(value) }} />
          </View>
          <TouchableOpacity className='ml-1 w-16 h-8 bg-orange-400 rounded-xl justify-center' onPress={handleSearch}>
            <Text className='text-center text-white text-xs font-bold'>Tìm kiếm</Text>
          </TouchableOpacity>
        </View>
        <View className='flex-1 pl-5 pt-2'>
          ...
          {(products.length <= 0) && <Text className='text-base font-bold mt-2' >Đề xuất tìm kiếm</Text> }
          <ScrollView className='gap-6 mt-2'>
